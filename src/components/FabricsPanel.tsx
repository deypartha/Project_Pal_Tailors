...
import { useFabrics } from '../lib/hooks';
import { formatPrice } from '../lib/format';
import ImageUploader from './ImageUploader';
import type { Fabric } from '../types';

const emptyDraft: Omit<Fabric, 'id' | 'created_at' | 'code'> = {
  name: '',
  description: '',
  price: null,
  image_url: '',
...
export default function FabricsPanel() {
  ...
  const toggleSoldOut = async (f: Fabric) => {
    await supabase.from('fabrics').update({ sold_out: !f.sold_out }).eq('id', f.id);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-display text-2xl font-semibold text-charcoal">Fabrics</h3>
          <p className="mt-0.5 text-sm text-sand-600">{fabrics.length} fabrics available</p>
        </div>
        <button
      ...
      <div className="relative mt-6 max-w-sm">
        ...
        <input
          ...
          placeholder="Search by code (FB-0001) or name…"
      ...
      {fabrics.length === 0 ? (
        <p className="mt-10 rounded-2xl bg-white p-10 text-center text-sand-600 shadow-soft">
          No fabrics yet. Click “Add Fabric” to create your first one.
        </p>
      ) : filtered.length === 0 ? (
        <p className="mt-10 rounded-2xl bg-white p-10 text-center text-sand-600 shadow-soft">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((f) => (
            ...
            >
              <div className={`relative aspect-square bg-sand-100 ${f.sold_out ? 'grayscale' : ''}`}>
                {f.image_url ? (
                  <img src={f.image_url} alt={f.name} className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center font-display text-4xl text-sand-300">
                    {f.name.charAt(0)}
                  </div>
                )}
                {f.sold_out && (
              ...
              <div className="flex flex-1 flex-col p-4">
                <h4 className="font-medium text-charcoal">{f.name}</h4>
                <p className="mt-0.5 line-clamp-1 text-xs text-sand-600">{f.description}</p>
                <div className="mt-2 text-xs text-sand-500">
                  {f.price != null ? formatPrice(f.price) : 'Price on request'}
                </div>
                ...
                <div className="mt-4 flex items-center gap-2">
                  ...
                  >
                    {f.sold_out ? 'Mark Available' : 'Mark Sold Out'}
                  </button>
                  <button
                    onClick={() => remove(f.id)}
...
function FabricFormModal({ fabric, onClose }: { fabric: Fabric | null; onClose: () => void }) {
  const [draft, setDraft] = useState<Omit<Fabric, 'id' | 'created_at' | 'code'>>(
    fabric
      ? {
          name: fabric.name,
  ...
  const handleSave = async () => {
    ...
    const { error } = fabric
      ? await supabase.from('fabrics').update(payload).eq('id', fabric.id)
      ...
      <div className="relative z-10 max-h-[88vh] w-full max-w-lg animate-scale-in overflow-y-auto rounded-t-3xl bg-sand-50 shadow-elevate sm:rounded-3xl">
          ...
          <div>
            ...
            {fabric?.code ? (
              <p className="mt-0.5 text-xs font-semibold tracking-wider text-sand-500">
                {fabric.code} · code auto-assigned
              </p>
            ) : (
              <p className="mt-0.5 text-xs text-sand-500">
                A code like FB-0001 will be auto-assigned on save
              </p>
            )}
          </div>
          <button
        ...
        <div className="sticky bottom-0 flex gap-3 border-t border-sand-200 bg-sand-50/95 px-6 py-4 backdrop-blur">
          ...
          >
            ...
            {fabric ? 'Save Changes' : 'Add Fabric'}
          </button>
        </div>
      </div>
    </div>
